...
import {
  ...
  ChevronRight, ChevronLeft, AlertCircle, Package, Sparkles,
} from 'lucide-react'
          ...
          <div key={num} className="flex items-center">
            <div className="flex flex-col items-center gap-1 w-10 sm:w-12">
              <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all" style={{
                ...
                boxShadow: active ? '0 0 0 3px rgba(74,127,165,0.18)' : 'none',
              }}>
                {done ? <Check size={12} /> : num}
              </div>
              <span className="text-[9px] sm:text-[10px] font-semibold tracking-wide text-center" style={{
                color: active ? '#0D1F3C' : done ? '#4A7FA5' : 'rgba(13,31,60,0.35)',
              }}>{label}</span>
            </div>
            {i < STEPS.length - 1 && (
              <div className="w-3 sm:w-8 h-0.5 mb-4 mx-0.5 transition-colors flex-shrink-0" style={{ background: done ? '#4A7FA5' : 'rgba(13,31,60,0.10)' }} />
            )}
          </div>
        )
      })}
...
const card: React.CSSProperties = { background: 'white', borderRadius: '20px', border: '1.5px solid rgba(13,31,60,0.07)', padding: 'clamp(1.1rem, 4vw, 1.75rem)' }
...
export default function BookingForm() {
  ...
  const onBookingTypeChange = (bt: 'one-time' | '10pack') => {
    setBookingType(bt)
  ...
  if (success) {
    return (
      <div className="max-w-2xl mx-auto">
        <div style={{ ...card, textAlign: 'center', padding: 'clamp(1.5rem, 5vw, 2.5rem)' }}>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {step === 1 && (
        <div style={card}>
          <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '4px' }}>Your Information</h2>
      ...
      {step === 2 && (
        <div style={card}>
          <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '4px' }}>Children</h2>
      ...
      {step === 3 && (
        <div className="space-y-5">
          {/* Booking type */}
          <div style={card}>
            <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '1.25rem' }}>How would you like to book?</h2>
            <div className="space-y-3">
              {([
                { value: 'one-time' as const, label: 'Single lesson',    desc: 'Book a specific date and time',                icon: <Check size={15} /> },
          ...
          {/* Lesson duration */}
          <div style={card}>
            <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '4px' }}>Lesson Duration</h2>
            ...
            <div className="grid grid-cols-2 gap-3">
              {LESSON_TYPE_OPTIONS.filter(lt => bookingType === '10pack' ? lt.pack : !lt.pack).map(lt => {
                ...
                return (
                  <button key={lt.value} type="button" onClick={() => setLessonType(lt.value)} style={{ borderRadius: '16px', padding: '0.875rem', cursor: 'pointer', textAlign: 'left', position: 'relative', border: selected ? '2px solid #0D1F3C' : '2px solid rgba(13,31,60,0.10)', background: selected ? 'rgba(13,31,60,0.03)' : 'white', transition: 'all 150ms' }}>
                    ...
                    <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '12px', fontWeight: 700, color: '#0D1F3C', marginBottom: '4px', paddingRight: '1.5rem' }}>{lt.label}</p>
                    <p style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: '17px', fontWeight: 800, color: selected ? '#0D1F3C' : '#4A7FA5', marginBottom: '4px' }}>{price}</p>
                    <p style={{ fontFamily: 'var(--font-dm-sans)', fontSize: '10px', color: 'rgba(13,31,60,0.38)' }}>{lt.hint}</p>
                  </button>
                )
              })}
            </div>
          ...
          {/* Lesson format */}
          <div style={card}>
            <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '1.25rem' }}>Lesson Format</h2>
      ...
      {step === 4 && (
        <div style={card}>
          <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '4px' }}>
      ...
      {step === 5 && (
        <div className="space-y-5">
          {needsAssignment && sessions.length > 0 && (
            <div style={card}>
              <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '4px' }}>Assign Children to Sessions</h2>
          ...
          <div style={card}>
            <h2 style={{ fontFamily: 'var(--font-fraunces, Georgia, serif)', fontSize: 'clamp(18px, 4vw, 22px)', fontWeight: 800, color: '#0D1F3C', marginBottom: '4px' }}>Review Your Request</h2>
            ...
            <div className="space-y-4">
              ...
              <div style={{ background: 'rgba(13,31,60,0.03)', borderRadius: '14px', padding: '1rem' }}>
                ...
                <div className="flex flex-wrap gap-2">
                  {[getLessonDuration(lessonType) === 45 ? '45-Minute Lesson' : '30-Minute Lesson', lessonFormat === 'semi-private' ? 'Semi-Private' : 'Private', bookingType === '10pack' ? '10-Pack' : 'One-time'].map(tag => (
